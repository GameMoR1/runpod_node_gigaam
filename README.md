# GigaAM CUDA FastAPI Service

FastAPI-сервис для CUDA-транскрибации на базе `gigaam` с очередью задач (1 GPU = 1 воркер), загрузкой enabled-моделей из БД и callback по завершению.

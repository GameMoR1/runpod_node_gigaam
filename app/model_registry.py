...
class ModelRegistry:
    ...
    async def load_from_db_and_prepare(self) -> None:
        ...
        if not self._models:
            raise RuntimeError("no enabled gigaam models")

        logger.info("enabled gigaam models: %d", len(self._models))
    ...
    async def _download_model(self, model_name: str) -> None:
        ...
        def _blocking_download() -> None:
            ...
            try:
                m = gigaam.load_model(model_name, device="cpu", download_root=str(cache_dir))
            except TypeError:
                try:
                    m = gigaam.load_model(model_name, device="cpu")
                except TypeError:
                    m = gigaam.load_model(model_name)
            del m
